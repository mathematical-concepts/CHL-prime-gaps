...
\date{Whitepaper v1.5 --- Final Candidate with CHL3 Negative-Control Remark}
\begin{document}
\maketitle
\begin{abstract}
\noindent\textbf{Whitepaper v1.5 --- Final Candidate with CHL3 Negative-Control Remark.}\par
...
\]
The scalar $\eta_Y(x)$ is a Lagrange multiplier used to impose the finite-window mean-gap scale. In the reported implementation, $\eta_Y$ is solved numerically within the evaluated support so that the model mean of $g_2$ matches the empirical/PNT local scale in that block and stratum. On a finite even support, the exponential-family log-partition function is strictly convex in $\eta_Y$ whenever the support has nonzero variance, and the corresponding mean is monotone in $\eta_Y$; hence the anchoring equation $\mathbb E[g_2]=\log x$ admits a unique solution, obtainable by bisection or Newton--Raphson. Thus $\eta_Y$ is not a fitted local feature: its value is determined by the PNT/empirical scale anchor within the finite support. While CHL1 captures path-dependent arithmetic weight, it does not enforce strict consecutiveness.
...
\subsection{Metrics}
The primary metric is conditional log-likelihood per event, $\loglik(M)=N^{-1}\sum_i \log P_M(g_{2,i}\mid g_{1,i},x_i)$. Conditional log-likelihood is used because it is additive across events, directly corresponds to cross-entropy minimization, and penalizes tail misalignment more sensitively than integral distance metrics in sparse regimes. We also report conditional cross-entropy and KL divergence. Pairwise gains are $\Delta\loglik(M,B)=\loglik(M)-\loglik(B)$. A positive value means that model $M$ assigns higher observed conditional likelihood than baseline $B$.
...
\begin{figure}[H]\centering\includegraphics[width=0.94\textwidth]{figures/fig1_chl2_main_gains.png}\caption{CHL2 path-exclusion gains by evaluation stratum.}\end{figure}
...
\begin{figure}[H]\centering\includegraphics[width=0.88\textwidth]{figures/fig2_y_sweep_chl2_vs_chl1.png}\caption{Cutoff stability of CHL2 path-exclusion.}\end{figure}
...
\begin{figure}[H]\centering\includegraphics[width=0.88\textwidth]{figures/fig4_y_sweep_baselines_all.png}\caption{Full-support cutoff-stable gains of CHL2 path-exclusion against selected baselines.}\end{figure}
...
\begin{figure}[H]\centering\includegraphics[width=0.78\textwidth]{figures/fig3_os_diagonal_probabilities.png}\caption{Absolute prime-residue transition diagnostic. CHL2 captures repulsion in $q=5,7$ but fails in $q=3$.}\end{figure}
...
\textbf{Research-outlook disclaimer.} This subsection is a research outlook; the expressions below are conjectural linearizations, not proven optimality conditions. Analytic obstacles in mapping probability distributions to sieve weights are acknowledged.
...
\subsection{Direct modular transfer as a separate problem}
The present paper studies consecutive prime gaps through conditional interior survival. In this framework, the CHL2 kernel estimates the probability that a proposed next gap survives the Hardy--Littlewood endpoint structure and contains no interior prime. This is a gap-survival problem.

The prime-residue transition matrices appearing in the Oliver--Soundararajan diagnostic suggest a related but distinct problem. For a modulus $q$, define
\[
T_q(b,a)=P(p_n\equiv a\pmod q\mid p_{n-1}\equiv b\pmod q),
\]
where $a,b$ range over reduced residue classes. CHL2 induces a prediction for $T_q$, but it is not designed as a direct transfer law on residue states. The failure in the binary reduced system modulo $3$ indicates that some micro-scale residue transitions may require a transfer mechanism not expressible purely as no-interior survival.

We therefore regard direct modular transfer kernels as a separate research direction. A future model would study the residual transition law
\[
\mathcal{T}_q(b,a)=\log T_q^{\mathrm{emp}}(b,a)-\log T_q^{\CHLtwo}(b,a),
\]
or an equivalent parameter-free arithmetic approximation to it. Such a programme should be developed separately from CHL2, because introducing a direct transfer factor into the present kernel would change the object under study: from survival inside a candidate gap to state-to-state modular transport.

This separation is deliberate. CHL2 remains a conditional Hardy--Littlewood gap-survival kernel. Direct modular transfer is a natural next problem, especially for low wheels such as $q=3$, but it should be analysed as a distinct mechanism rather than retrofitted into the present model.
...
\subsection*{A.4. Remark on higher-order low-wheel sieve corrections}

A natural question is whether the $q=3$ anomaly can be removed by replacing the first-order Poisson no-interior approximation with a stricter inclusion--exclusion correction. We investigated this possibility experimentally through second-order low-wheel cluster cumulants. In this variant, unordered pairs of interior candidates $u < v$ are evaluated through the quintic extension
\[
H_5(u,v)=\{0,g_1,g_1+u,g_1+v,g_1+g_2\},
\]
and configurations that exhaust the reduced residue space modulo small primes are assigned hard zeros in the corresponding low-wheel singular factor.

This correction behaves in the expected algebraic direction. When $H_5(u,v)$ becomes inadmissible in the low wheel, the joint contribution vanishes and the pair cumulant becomes negative:
\[
\kappa_{uv}=p_{uv}-p_up_v=-p_up_v.
\]
Since the second-order intensity has the form
\[
\Omega_Y^{(2)}=\Omega_Y^{(1)}-\sum_{u < v}\kappa_{uv},
\]
such negative cumulants increase the no-interior intensity and therefore lower the survival probability. Thus the strict low-wheel correction correctly identifies mutually exclusive interior configurations.

However, our experimental evaluation showed that these negative hard-zero contributions are largely offset by positive cumulants from pairs occupying compatible or repeated low-wheel residue classes. Consequently, the correction slightly improves some conditional likelihood scores but does not reverse the qualitative $q=3$ sign error. The empirical transition remains diagonally repulsive, while the corrected no-interior model still predicts diagonal persistence.

This suggests that the $q=3$ anomaly is not merely an artifact of using a first-order Poisson truncation for interior-prime exclusion. In the compressed two-state reduced residue system modulo $3$, pairwise interior exclusion detects local impossibilities but does not encode the full global capacity constraint of the residue space. By the pigeonhole principle, saturation effects in such a binary system are collective: isolated pair cumulants can register hard zeros, but they do not necessarily generate the correct state-to-state transfer law.

We therefore interpret the $q=3$ failure as evidence for a missing direct modular transfer mechanism, rather than as a defect that can be repaired by no-interior sieve exclusion alone in the first- and second-order forms tested here. This does not affect the conditional gap-likelihood results of CHL2, nor the stronger agreement observed for $q=5$ and $q=7$; it identifies a sharper boundary between interior survival modelling and direct modular residue transfer.

\subsection*{A.5. Consequence for the claims of the paper}
...
This distinction is essential.  CHL2 is a strong first-order conditional kernel for consecutive prime gaps; it is not a complete model of every low-modulus residue bias.  The second-order low-wheel follow-up described above did not repair the $q=3$ wrong-sign effect.  We therefore regard direct modular transfer, rather than additional no-interior survival terms alone, as the appropriate next object of study.

\section{Reproducibility summary}
The CHL2 audits used ten parent-wide DS1 blocks and the real chronological prime sequence for the absolute residue diagnostic. The main run used $Y=47$ as truncation horizon, path-exclusion enabled, and all blocks B01--B10. The cutoff sweep used $Y\in\{31,47,61,73\}$. The OS absolute run used $q=3,5,7$ and the model \texttt{CHL2\_path\_excl\_cond\_eta}. The path-exclusion cache is an acceleration artifact and not a fitted model object. Repository scripts also write runtime telemetry JSON files recording elapsed wall-clock time, command-line arguments, CPU count, truncation horizon, block counts, and output-row counts. These telemetry files are reproducibility aids only; they are not used by the mathematical kernels and do not affect any reported probability or likelihood.
