...
\[
p_u = \frac{1}{\log x}\frac{\mathfrak S_Y(H_4(u))}{\mathfrak S_Y(H_3)},
\qquad
...
\[
\mathfrak S_{\rm low}(H_5)=0,
\]

so the low-wheel coupling is zero and

\[
\kappa_{uv}=-p_u p_v.
\]

This negative cumulant increases \(\Omega^{(2)}\) and decreases survival.

## Hybrid design

The intended CHL3 low-wheel design is hybrid:

- one-point intensities \(p_u\) are still full CHL2/H4/H3 quantities;
- pairwise covariance is corrected only through selected low primes, e.g.
  \(\{3\}\), \(\{3,5\}\), or \(\{3,5,7\}\);
- the hard-zero condition for these low primes is checked explicitly.

This avoids an exponential Möbius expansion while preventing q=3 hard zeros from
being averaged away.
